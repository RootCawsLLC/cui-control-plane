...
| 12 | **A fixture run overwrites real evidence** | open defect | `pipeline --fixture` writes to the same `evidence.path` and the same `<control>@<date>.json` filename as a live run, so a demo silently destroys that day's real evidence — and `.evidence/` is gitignored, so git cannot restore it. Needs separate fixture output, or a refusal to overwrite non-fixture evidence. | nothing yet — this row is the only thing standing between the behaviour and a surprise |

...
| 12 | **A retention shortfall cannot name its own cause** | by design, for now | `ccp doctor` reports the span held against `evidence.retain_days`, but cannot tell "the deployment is younger than the commitment" from "evidence that existed was removed". Settling it needs a manifest of expected snapshots — a control cadence plus a start date — so absence becomes checkable. Until then the check names both possibilities rather than guessing. | `describeRetention` in `src/retention.mjs`, and a test asserting the wording accuses nobody |
